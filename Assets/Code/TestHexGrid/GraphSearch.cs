...
{
    public static BFSResult BFSGetRange(HexGrid hexGrid,Vector3Int startPoint,int movementPoints,bool editCost)
    {
        ...
        while (nodesToVisitQueue.Count > 0)
        {
            ...
            foreach(Vector3Int neighbourPosition in hexGrid.GetNeighboursFor(currentNode))
            {
                ...
                if (editCost)
                { nodeCost = 1; }
                else { nodeCost = hexGrid.GetTileAt(neighbourPosition).GetCost(); }
